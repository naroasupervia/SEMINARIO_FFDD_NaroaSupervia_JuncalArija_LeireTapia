...
output:
  html_document:
    ...
    toc: true             # Añade tabla de contenidos
    toc_float: true       # TOC flotante que se desplaza al hacer scroll
    number_sections: true # Numera capítulos y secciones
    fig_width: 7
    fig_height: 5
...
El **suicidio** es una de las principales causas de muerte no natural a nivel global y constituye un importante problema de salud pública en España.

En este seminario, nos proponemos estudiar como ha evolucionado el suicidio en España  en los ultimos años utilizando datos oficiales del INE para explorar las diferencias entre el sexo la edad y el territorio, entre otras, con la intención de entender mejor qué tendencias aparecen en los datos y que factores podrían estar influyendo en estas variaciones.
...
4. Unificar los datos CSV y JSON para obtener una vista más completa y comparar la relación y fiabilidad de los datos.

5. Analizar la frecha de género 

```{r, echo=FALSE, fig.align='center', out.width="50%"}
knitr::include_graphics("INPUT/DATA/imagen_fuentes.jpg")
```
...
- **suicidio_Edad-Sexo.json**: Este JSON  contiene el número absoluto de suicidios registrados a nivel nacional en el año 2022. Está desglosado por grupo de edad y por sexo. 
...
En algunos de nuestros archivos csv, es necesario especificar el tipo de codificación que tienen, para que R pueda leer correctamente los caracteres especiales como *tildes* o *ñ*. 
...
suicidios_json <- jsonlite::fromJSON("INPUT/DATA/Suicidio_Edad-Sexo.json", flatten=TRUE)

suicidios_df <- as.data.frame(suicidios_json)
...
Los datos han sido filtrados y preparados para asegurar la coherencia entre las fuentes. 

### **Convertir la columna 'Total' del CSV de texto a número**
```{r , eval = TRUE,message=FALSE}
provincias <- provincias %>%
  mutate(
    #Cambiamos total a numerico y la coma a punto
    Total = as.numeric(gsub(",", ".", Total)),
    # Convertimos'Periodo' a número también
...
Suicidios_Edad <- as_tibble(suicidios_json) %>%
  ...
  # Selecciona solo las columnas que necesitamos para pivotar
  select(ID_Fila, Valor, T3_Variable, Nombre) %>%

  # Pivotar usando el ID_Fila
  ...
  # Quitar la columna de ID, ya no la necesitamos
...
## RESULTADOS 
Estudio de los datos y gráficos 
...
Podemos representar los datos anteriores en un gráfico circular para obtener otra visualización.
...
Los resultados de los gráficos muestra que los hombres presentan claramente un mayor número de suicidios. Podemos decir que los hombres representan aproximadamente el 75% de los suicidios a nivel nacional, mientras que las mujeres el 25%. 
...
Suicidios_CSV_filtrado_FINAL <- provincias %>%
  filter(Sexo != "Total",
         Provincias != "Nacional") %>%
  mutate(
    # Eliminar código numérico inicial
    ...
    Nombre_Final = case_when(
        ...
        # Convertir el resto a formato Título 
...
theme(
  ...
  strip.text = element_text(size = 7), # Tamaño de nombres de provincias
...
# Conversión a interactivo y ajuste final
p_plotly <- ggplotly(p)
...
El gráfico muestra la tasa de suicidios por cada 100.000 habitantes en todas las provincias españolas entre 2021 y 2023, y además separa los datos por sexo (hombres en rojo y mujeres en azul), lo que permite ver tanto las diferencias entre provincias como la brecha entre hombres y mujeres.

Al observar el gráfico podemos ver que existe una diferencia muy grande entre hombres y mujeres, y que las tasas cambian mucho según la provincia, sobre todo en los hombres.

En cuanto a la diferencia entre sexos, las mujeres tienen tasas bastante bajas y estables, normalmente entre 5 y 7 casos por cada 100.000 habitantes, mientras que los hombres, presentan valores mucho más altos y con más variación, llegando incluso a triplicar o cuadruplicar las cifras femeninas en la mayoría de los territorios.

También se ven diferencias claras entre provincias, especialmente entre los hombres. Las tasas masculinas más altas aparecen normalmente en provincias como Asturias, Lugo, Ourense o Zamora que superan o se acercan a los 20 casos por cada 100.000 habitantes, y en otras provincias, como Madrid, Guadalajara o Valladolid, las tasas son más bajas (alrededor de 10-12), aunque la diferencia respecto a las mujeres sigue existiendo.
...
# Agregación y preparación de códigos
...
# Descargar geometría y unir datos
...
Este mapa muestra cómo se distribuyen las tasas medias de suicidio por cada 100.000 habitantes en España entre 2021 y 2023, haciendo media con hombres y mujeres. Los colores ayudan a identificar rápidamente las zonas con más o menos incidencia, ya que los tonos claros representan tasas bajas y los tonos rojos más intensos indican tasas altas.

Al analizar el mapa, podemos ver que las tasas más altas se concentran sobre todo en el noroeste, en zonas de Galicia, Zamora y León, especialmente Lugo aparece con el tono más oscuro, lo que indica que están muy por encima de la media nacional, con valores que rondan o superan los 12-14 casos por cada 100.000 habitantes, siendo esta la provincia con mayor tasa de suicidios.

Por el contrario, en zonas del centro como Madrid, tienen tasas bastante más bajas, alrededor de 4 a 8 casos.

### **Suicidio por edad (JSON)**
```{r , eval = TRUE,message=FALSE}
# 1. Definimos el orden correcto cronológicamente
orden_correcto <- c(
  "Menores de 15 años",
  "De 15 a 29 años",
  "De 30 a 39 años",
...
Suicidios_Edad$edad <- factor(Suicidios_Edad$edad, levels = orden_correcto)

ggplot(Suicidios_Edad, aes(x = edad, y = Valor, fill = sexo)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(x = "Grupo de Edad", y = "Número de Suicidios")
```

Este gráfico de barras muestra cuántos suicidios hubo en total según los diferentes grupos de edad, separando también por sexo (hombres en rojo y mujeres en azul). A diferencia de los mapas o las tasas por provincias, aquí no hablamos de proporciones, sino de números absolutos. Eso permite ver claramente en qué etapas de la vida se concentran más casos.

A partir del gráfico se pueden destacar dos ideas principales, la brecha entre hombres y mujeres aparece en todas las edades, y los suicidios se concentran sobre todo en la mediana edad.

Las edades comprendidas entre los 40 y los 59 años son las que acumulan más casos tanto en hombres como en mujeres. En especial, los grupos de 50-54 y 55-59 años son los que muestran los valores más altos.

En cambio, las cifras descienden bastante en los grupos de mayor edad de 90 a +95 años y son muy bajas entre los menores de 15.
...
**1. Creación de la tabla de referencia**
...
# 2. Gráfico de Piruleta para visualizar la diferencia
...
# CONCLUSIONES
Tras el análisis comparativo de las tasas de suicidio en España utilizando datos oficiales del INE, podemos extraer las siguientes conclsiones principales: Se ve claramente que el suicidio en España no afecta igual a todo el mundo. Por un lado, hay una diferencia muy marcada entre hombres y mujeres, ya que los hombres siempre presentan cifras más altas, tanto en tasas por 100.000 habitantes como en números absolutos. 
Por otro lado, tampoco todas las provincias se comportan igual, en zonas del noroeste es donde aparecen las tasas más elevadas, mientras que en el centro y el sur los valores son bastante más bajos en comparación.

Si miramos por edad, los casos se concentran sobre todo en la mediana edad, especialmente entre los 40 y los 59 años. En cambio, en los grupos más jóvenes y más mayores los valores bajan bastante.

Además, los datos del CSV y del JSON encajan bien entre sí, lo que da más confianza en los resultados del análisis. Gracias a esto, podemos entender mejor un problema tan complejo y ver que influyen muchos factores como el lugar donde se vive, la edad y el sexo.

Este análisis de datos destaca la necesidad de estrategías de prevención dirigidas específicamente a los grupos de mayor riesgo identificados. Que en base a este estudio serían los hombres de Lugo que están en la mediana edad.
